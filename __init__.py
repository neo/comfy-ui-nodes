    ...
    def INPUT_TYPES(s):
        ...
        return {
            "required": {
                "aspect_ratio": (
                    dimensions,
                    {"default": dimensions[len(dimensions) // 2]},
                ),
...
class EmptyLatentImageSDXL:
    ...
    def generate(self, aspect_ratio, batch_size=1):
        width, height = self.SUPPORTED_DIMENSIONS[aspect_ratio]
        latent = torch.zeros(
            [batch_size, 16, height // 8, width // 8], device=self.device
        )
        return ({"samples": latent},)
...
class ToggleDifferentStepsCfgSet:
    ...
    def choose(self, steps, cfg, use_the_other_set, the_other_steps, the_other_cfg):
        return (
            (steps, cfg) if not use_the_other_set else (the_other_steps, the_other_cfg)
        )
